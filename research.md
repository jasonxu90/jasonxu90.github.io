...
Shape-constrained regression and density estimation are classical tasks in statistics, but algorithms for the multivariate setting lag behind recent theoretical advances. Convexity is the simplest shape restriction that enjoys a natural generalization to higher dimensions. Regression subject to convexity restrictions presents computational difficulties, as the set of constraints to be enforced increases rapidly with the number of observations. 
